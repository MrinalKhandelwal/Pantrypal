...
export default function Home() {
  ...
  const [error, setError] = useState(""); //wrong input ke liye error message store karega
  ...
  const removeIngredient = (index) => {
   ...
   // Recipe ke ingredients ko user ke ingredients ke saath compare karega
    ...
    setLoading(true);
    setError(""); //purana error message hata dega
    setRecipes([]); //purani recipes clear karega
    ...
    return fetch(
        ...
        // Agar koi recipe nahi mili toh wrong ingredient ka error show karega
        if (allRecipes.length === 0) {
          setError(
            `❌ No recipes found for "${ingredients.join(", ")}". Please enter a valid ingredient.`
          );

          setLoading(false); //loading band karega
          return; //aage ka code run nahi karega
        }
  ...
  return (
    <div className="container-fluid py-5" style={{backgroundColor: "#fff",minHeight: "100vh", width: "100%"}}>
                ...
                <div className="d-flex flex-wrap justify-content-center gap-2 mb-4">
                  ...
                  {/*Wrong ingredient hone par error message show hoga*/}
                  {error && (
                    <div
